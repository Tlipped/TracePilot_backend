...
- **自动化审查接口**：基于日志、宏观分析结果和最终报告做跨 Agent 一致性检查，辅助发现结论漂移。
- **可复现审计平台接口**：支持宏观分析结果、自动化审查、分页历史日志、完整日志、Agent 文件日志、归档和删除等 API。
...
| `GET` | `/api/tasks/{task_id}/automated-review` | 获取后端自动化审查结果，检查跨 Agent 证据链一致性 |
