...
TOKEN20_721_TRANSFER_TOPIC = '0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef'


def parse_token_transfer(log: dict) -> Union[dict, None]:
    if log['topics'][0] == TOKEN20_721_TRANSFER_TOPIC:
        # load log topics
        topics = log.get('topics')
        if any([
            topics is None,
            len(topics) < 1,
            topics[0] != TOKEN20_721_TRANSFER_TOPIC
        ]):
            return
        ...
        if len(topics_with_data) != 4:
            return
        return {
            "from": word_to_address(topics_with_data[1]),
            "to": word_to_address(topics_with_data[2]),
...
def parse_bytes_data(data: bytes, output_types: list) -> Union[tuple, None]:
    ...
    :return:
    """
    if not isinstance(data, str) or data == '0x' or not data:
        return None
